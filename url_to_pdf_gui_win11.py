  ...
  class PDFConverter:
      ...
      def create_widgets(self):
          ...
          if self.wkhtmltopdf_path:
              ...
              ttk.Label(main_frame, text=status_text, foreground="green").grid(row=1, column=0, columnspan=3,
  pady=(0, 15))
          else:
              status_text = "❌ wkhtmltopdf が見つかりません"
              ttk.Label(main_frame, text=status_text, foreground="red").grid(row=1, column=0, columnspan=3, pady=(0,
   5))

              help_text = "インストール: https://wkhtmltopdf.org/downloads.html"
              ttk.Label(main_frame, text=help_text, foreground="blue").grid(row=2, column=0, columnspan=3, pady=(0,
  15))
